...
<a href="<?php echo U('Ptsys/catAdd', array('token' => $token, 'cid' => $cid, 'parentid' => $parentid));?>" title="新增分类" class="btnGrayS vm bigbtn"><img src="<?php echo RES;?>/images/product/add.png" class="vm">新增<?php if($_GET['parentid'] != 0 and $_GET['parentid'] != ''): ?>子<?php endif; ?>分类</a>
</div>
<?php if($parentid > 0): ?><div class="pageNavigator right">
<a href="<?php echo U('Ptsys/cat',array('token'=>$token, 'cid' => $cid, 'parentid' => $parentCat['parentid']));?>" class="btnGrayS vm bigbtn">返回上级分类</a>
</div>
<?php else: ?>
...
<td><?php echo ($hostlist["cat_name"]); ?></td>
<td><?php echo ($hostlist["sort_order"]); ?></td>
<td>
	<?php if($hostlist["is_show"] == 1): ?><span>是</span>
		<?php else: ?>
		<span style="color:red">否</span><?php endif; ?>
</td> 

<td class="norightborder">
	<a href="<?php echo U('Ptsys/catSet',array('token'=>$token,'id'=>$hostlist['cat_id'],'parentid' => $hostlist['parent_id']));?>">修改</a>

	<a href="javascript:drop_confirm('您确定要删除吗?', '<?php echo U('Ptsys/catDel',array('id'=>$hostlist['cat_id'],'token'=>$token));?>');">删除</a>

	<span>&nbsp;|&nbsp;
	<a href="<?php echo U('Ptsys/index',array('catid' => $hostlist['id'],'token'=>$token,'cat_id' => $hostlist['cat_id']));?>" style="color:#f00">商品管理</a></span>
</td>
