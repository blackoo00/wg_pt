...
<!-- <h4 class="left">团购订单（<a href="<?php echo U('Ptsys/order',array('token'=>$token,'handled'=>0));?>">失败待退款订单<span style="color:#f00"><?php echo ($unhandledCount); ?></span>个</a>） (<?php echo ($page); ?>) 
</h4> -->
...
<li class="current tabli" id="tab2"><a href="<?php echo U('PtOrder/team',array('token'=>$token,'dining'=>$isDining));?>">团购列表</a></li>
<li class="tabli" id="tab2"><a href="<?php echo U('PtOrder/order',array('token'=>$token,'dining'=>$isDining));?>">订单列表</a></li>
...
<option value="0" <?php if(($_REQUEST['team_status']) == "0"): ?>selected<?php endif; ?>>团购等待开团</option>
<option value="1" <?php if(($_REQUEST['team_status']) == "1"): ?>selected<?php endif; ?>>团购正在进行中</option>
<option value="2" <?php if(($_REQUEST['team_status']) == "2"): ?>selected<?php endif; ?>>团购成功</option>
<option value="3" <?php if(($_REQUEST['team_status']) == "3"): ?>selected<?php endif; ?>>团购失败待退款</option>
<option value="4" <?php if(($_REQUEST['team_status']) == "4"): ?>selected<?php endif; ?>>团购失败已退款</option>
</select>
...
<td>
	<?php switch($o["team_status"]): case "0": ?>初始状态<?php break;?>
		...
		<?php case "2": ?><span style="color:blue">成功</span><?php break;?>

		<?php case "3": ?><span style="color:red">失败待退款</span><?php break;?>
...
<td>
	<?php echo ($o['team_num']-$o['teammen_num']); ?>
</td>

<td>
	<?php echo (date("Y-m-d h:i:s",$o["pay_time"])); ?>
</td>


<td>

	<?php echo (date("Y-m-d h:i:s",$o['pay_time']+86400)); ?>

</td>
<td class="norightborder">
<!-- <a href="###" onclick="showIntroDetail(<?php echo ($o["order_id"]); ?>)">详细</a> 
 -->&nbsp;&nbsp;
<a href="<?php echo U('PtOrder/order',array('teamid' => $o['team_sign']));?>">团员订单详情</a>
<!-- <a href="javascript:drop_confirm('您确定要删除吗?', '<?php echo U('Store/deleteOrder',array('token'=>$token,'id'=>$o['id'],'dining'=>$isDining));?>');">删除</a></td>  -->
...
function showIntroDetail(id){
	art.dialog.open('<?php echo U('Ptsys/teamInfo',array('token'=>$token,'dining'=>$isDining));?>&id='+id,{lock:false,title:'订单详情',width:1000,height:620,yesText:'关闭',background: '#000',opacity: 0.87,close: function(){location.reload();}});
}
</script>
