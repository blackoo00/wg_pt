...
<div class="price">
<span class="okp">全国统一价：<em id="xsprice">￥<?php echo ($product["price"]); ?></em></span>
<span class="okp">正式分店价：<em id="vprice">￥<?php echo ($product["vprice"]); ?></em></span>
...
function Collection() {
	$.ajax({
		url: "<?php echo U('Store/addCollection',array('token'=>$token,'id'=>$product['id'],'wecha_id'=>$_GET['wecha_id']));?>",
		success: function(data) {
			if(data){
				if(data==1){
