...
class DistributionAction extends WapAction{
    ...
    public function __construct(){
		...
		if(!$my){
			if(ACTION_NAME!='autoHandle'&&ACTION_NAME!='myScanner'){
	...
	public function index(){
		$token		= $this->_get('token');
		$wecha_id	= $this->wecha_id;
		$totalScore = M('product_cart')->where(array('token'=>$token,'wecha_id'=>$wecha_id,'paid'=>1,'returnMoney'=>0))->sum('price');
	...
	public function myAddress(){
		...
		$db=M("address_list");
		$list=$db->where(array('mid'=>$this->my['id']))->order('choose desc,id desc')->select();
		if($list){
	...
	public function addAddress(){
		//城市区域
		$count=M("Address_list")->where(array('mid'=>$this->my['id']))->count();
		if($count>=5){
			$this->error('操作失败,每人最多可添加5条收货地址');
		}
		$db = D("Address");
		...
		if(IS_POST){
			if ($db->create() === false) {
			    ...
			    if ($id == true) {
			        ...
			        $this->success('操作成功', U("Distribution/myAddress",array("token"=>$this->token,'id'=>$_GET['id'],'did'=>$_GET['did'],'count'=>$_GET['count'])));
			    } else {
			        $this->error('操作失败', U("Distribution/myAddress",array("token"=>$this->token,'id'=>$_GET['id'],'did'=>$_GET['did'],'count'=>$_GET['count'])));
			    }
			}
		}else{
		...
		if(IS_POST){
			if ($db->create() === false) {
			    ...
			    if ($id == true) {
			        ...
			        $this->success('操作成功', U("Distribution/myAddress",array("token"=>$this->token,'id'=>$_GET['aid'],'did'=>$_GET['did'],'count'=>$_GET['count'])));
			    } else {
			        $this->error('操作失败', U("Distribution/myAddress",array("token"=>$this->token,'id'=>$_GET['aid'],'did'=>$_GET['did'],'count'=>$_GET['count'])));
			    }
			}
		}else{
			$id=$this->_get("id");
	...
	public function getMoney(){
		...
		if($this->my['distritime']==0){
			...
			$data = '{"touser":"'.$wecha_id.'","msgtype":"news","news":{"articles":[{"title":"'.$set['fxtitle'].'","description":"'.$set['fxtext'].'","url":"'.$set['fxurl'].'","picurl":"'.$set['fxpic'].'"]}}';
			$result = $this->api_notice_increment('https://api.weixin.qq.com/cgi-bin/message/custom/send?access_token='.$access_token,$data);*/
			$url = U('Distribution/myDistribution',array('token'=>$token,'wecha_id'=>$wecha_id,'notBe'=>1));
			$this->redirect($url);//跳回
			exit();
		}
		$id = $this->my['id'];
		...
		if(IS_POST){
			...
			if($money%10!=0){
				$arr = array('success'=>-1,'info'=>'每次提现金额必须为10的整数倍');
				echo json_encode($arr);
				exit;
			}
	...
	public function makeCode($token,$wecha_id,$mid){
		...
		$qrcode_url='https://api.weixin.qq.com/cgi-bin/qrcode/create?access_token='.$access_str;
