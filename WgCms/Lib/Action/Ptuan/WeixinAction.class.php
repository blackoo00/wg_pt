...
class WeixinAction extends BaseAction{
	...
	public function new_pay(){
		...
		if (!isset($_GET['code'])){
			//触发微信返回code码
			$url = $jsApi->createOauthUrlForCode(urlencode($this->siteUrl.'/wxpay/index.php?g=Ptuan&m=Weixin&a=new_pay&price='.$_GET['price'].'&orderName='.urlencode($_GET['orderName']).'&single_orderid='.$_GET['single_orderid'].'&showwxpaytitle=1&from='.$_GET['from'].'&token='.$_GET['token'].'&wecha_id='.$_GET['wecha_id']));
		...
		$unifiedOrder->setParameter("notify_url",C('site_url').'/wxpay/notice_pt.php');//通知地址 
		...
		$returnUrl = $this->siteUrl.'/index.php?g=Ptuan&m='.$from.'&a=payReturn&token='.$_GET['token'].'&wecha_id='.$_GET['wecha_id'].'&orderid='.$orderid;
...
<script language="javascript">function callpay(){WeixinJSBridge.invoke("getBrandWCPayRequest",'.$jsApiParameters.',function(res){WeixinJSBridge.log(res.err_msg);if(res.err_msg=="get_brand_wcpay_request:ok"){document.getElementById("payDom").style.display="none";document.getElementById("failDom").style.display="none";document.getElementById("successDom").style.display="block";window.location.href = \''.$returnUrl.'\';}else{if(res.err_msg == "get_brand_wcpay_request:cancel"){var err_msg = "您取消了支付";document.getElementById("action").innerHTML="重新支付";}else if(res.err_msg == "get_brand_wcpay_request:fail"){var err_msg = "支付失败<br/>错误信息："+res.err_desc;}else{var err_msg = res.err_msg +"<br/>"+res.err_desc;}document.getElementById("payDom").style.display="none";document.getElementById("failDom").style.display="";document.getElementById("failRt").innerHTML=err_msg;}});}</script>
</body>
</html>';
	}
	public function pay(){
		...
		$wxPayHelper->setParameter("notify_url", $this->siteUrl.'/index.php?g=Ptuan&m=Weixin&a=return_url&token='.$_GET['token'].'&wecha_id='.$_GET['wecha_id'].'&from='.$_GET['from']);
	...
	public function new_return_url (){
		...
		if(intval($_GET['total_fee']) && !intval($_GET['trade_state'])) {
			...
			//订单处理
			$order = D('Order_info')->where(array('orderid'=>$out_trade_no))->relation(true)->find();
			if($order['setInc']==0){
				D('Order_info')->where(array('orderid'=>$out_trade_no))->setField('order_status',1);
				//判断是团订单还是个人订单
				if($order['team_sign']){
					//判断是开团还是参团
					if($order['team_first'] == 1){
						...
						if($re && $team_data['team_status'] == 2){
							//发送拼团成功信息
							$team_orders = D('Order_info')->where(array('team_sign'=>$order['team_sign']))->relation(true)->select();
	...
	function distriOrderStatus($token,$order_id,$status) {
		...
		//删除库存
		//增加已售
		/*$carts = M('Product_cart_list')->where('cartid='.$order_id)->select();
		foreach ($carts as $key => $value) {
			M('Product')->where(array('id' => $value['productid']))->setDec('num', $value['total']);
			M('Product')->where(array('id' => $value['productid']))->setInc('fakemembercount', $value['total']);
			//增加真实销量
			M('Product')->where(array('id' => $value['productid']))->setInc('salecount', $value['total']);
		}*/
	}
	function infoSend($token,$orderid,$order_id,$access_token) {
		...
		foreach($orders as $key=>$value){
			...
			$url = "<a href='".C('site_url')."/index.php?g=Ptuan&m=Distribution&a=followOrder&token=".$token."&wecha_id=".$wecha_id."'>查看</a>";
