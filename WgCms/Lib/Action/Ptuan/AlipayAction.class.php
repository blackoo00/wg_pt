...
class AlipayAction extends BaseAction{
	...
	public function pay(){
		...
		if ($alipayConfig['paytype']=='Weixin'){
			Log::write('test=2','DEBUG');
			header('Location:/index.php?g=Ptuan&m=Weixin&a=pay&price='.$price.'&orderName='.$orderName.'&single_orderid='.$orderid.'&showwxpaytitle=1&from='.$from.'&token='.$this->token.'&wecha_id='.$this->wecha_id);
		}else {
			header('Location:/index.php?g=Ptuan&m='.$alipayConfig['paytype'].'&a=pay&price='.$price.'&orderName='.$orderName.'&single_orderid='.$orderid.'&from='.$from.'&token='.$this->token.'&wecha_id='.$this->wecha_id);
		}
	}
}
?>