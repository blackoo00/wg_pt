...
class AlipaytypeAction extends BaseAction{
	...
	public function pay(){
		...
		//必填，不能修改
		//服务器异步通知页面路径
		$notify_url = C('site_url').'/index.php?g=Ptuan&m=Alipaytype&a=notify_url';
		//需http://格式的完整路径，不能加?id=123这类自定义参数
		//页面跳转同步通知页面路径
		$return_url = C('site_url').'/index.php?g=Ptuan&m=Alipaytype&a=return_url&token='.$_GET['token'].'&wecha_id='.$_GET['wecha_id'].'&from='.$_GET['from'];
	...
	public function setconfig(){
		...
		$alipay_config['cacert']    = getcwd().'\\WgCms\\Lib\\ORG\\Alipay\\cacert.pem';
	...
	public function return_url (){
		...
		//if($verify_result) {
			...
			if( $this->_get('trade_status') == 'TRADE_FINISHED' ||  $this->_get('trade_status') == 'TRADE_SUCCESS') {
				...
				$this->redirect('/index.php?g=Ptuan&m='.$from.'&a=payReturn&token='.$orderInfo['token'].'&wecha_id='.$orderInfo['wecha_id'].'&orderid='.$out_trade_no);
			}else {
			  exit('付款失败');
			}
		//}else {
