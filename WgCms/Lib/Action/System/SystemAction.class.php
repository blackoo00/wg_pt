  ...
  PRIMARY KEY (`id`)
	...
	public function doUpdate(){
		...
		if (!isset($_GET['ignore'])){
			if (!is_writable($_SERVER['DOCUMENT_ROOT'].'/WgCms')){
				$cannotWrite=1;
				$this->error('您的服务器WgCms文件夹不可写入，设置好再升级',U('System/main'));
			}
			if (!is_writable($_SERVER['DOCUMENT_ROOT'].'/WgCms/Lib/Action')){
				$cannotWrite=1;
				$this->error('您的服务器/WgCms/Lib/Action文件夹不可写入，设置好再升级',U('System/main'));
			}
